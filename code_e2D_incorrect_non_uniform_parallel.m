...
% Incorrect Fast Parallel 2-D code
% Natural units
...
dzig=sqrt(1-ctheta3.^2)*1./(1-ctheta3).^2;      %Rutherford scattering
...
while i < n
    ...
    a = sqrt(1-(m1/e1)^2);
    b = sqrt(1-(m2/e2(i))^2);                   %non-uniform incidence angles
    if a < 0.999*b
...
parsave(sprintf('e2D_incoming_non_uniform_parallel_100thsnd%d.mat', ii), Eout2,Vout);
end
delete(gcp)