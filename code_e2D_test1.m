...
KT = 0.001;                                       %Temperature when BBN gets over
m1 = 100;                                         %Masses of particles
m2 = 1;
e1 = m1;                                        %Nuclei energy we start with
c = 1;                                          %speed of light
...
%     if a<b
%         theta1 = randpdf((b-a*cos(theta1)),theta1,[1,1]);
%     else
%         theta1 = randpdf((a-b*cos(theta1)),theta1,[1,1]);
%     end
%     theta1 = randpdf(sqrt(a^2 + b^2 - 2*a*b*cos(theta1) - a^2*b^2*sin(theta1).^2)./(1-a*b*cos(theta1)),theta1,[1,1]);
...
% x(1)=[];
% 
% clearvars -except Eout Eout2 Vout MJ2 MB2 KE
% save data_ee2D_test_with_xsection4.mat